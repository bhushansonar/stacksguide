...
if (!function_exists('get_admin_email')) {
    ...
    function get_admin_detail() {
        $array = array('email' => 'vijay@amutechnologies.com', 'name' => 'StacksGuide!');
        return $array;
    }
...
if (!function_exists('show_meta')) {
    ...
    function show_meta() {
        ...
        $ci = & get_instance();
        $user_id = $ci->session->userdata("user_id");
        $default_title = 'Stackguide';
//		$default_description = 'It`s like your own dog trained to bring your newsletters when you need them, to the place you want them... And from now on StacksGuide will protect your mailbox from spam and virus.';
//		$default_keyword = 'Newsletter , newsletter, get newsletter';
        ...
        return $return .='<title></title>
			...
			<meta property="og:site_name" content="Stackguide" />
        ...
        if (@is_array($cat_assoc_arr[$parent_id])) {
            foreach ($cat_assoc_arr[$parent_id] as $Pid => $db_cat_rs) {
                ...
                $par_cat_array .= '</div>
                                            ...
                                            <div class="timestamp"><a style="margin:6px" class="cmt_value" href="javascript:void(0)" onclick="javascript:reply_comment(this, ' . $db_cat_rs['comment_id'] . ');" id="rply_comment">Reply</a></div>
                                            <div class="timestamp">Commented On' . date('j, F Y', strtotime($db_cat_rs['date'])) . ' </div>';
