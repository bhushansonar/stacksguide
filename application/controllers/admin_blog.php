    ...
    public function add() {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST') {
            ...
            if ($this->form_validation->run()) {
                ...
                $data = $this->functions->do_upload('featured_image', './uploads/blog', $allowtype = 'gif|jpg|png', $maxWidth = "0", $maxHeight = "0", $minWidth = "0", $minHeight = "0");
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST') {
            ...
            if ($this->form_validation->run()) {
                ...
                if (isset($data['upload_data'])) {
                    $file_name = $data['upload_data']['file_name'];
                    $_POST['featured_image'] = $file_name;
                    @unlink("./uploads/blog/" . $this->input->post('old_featured_image'));
                } else {
