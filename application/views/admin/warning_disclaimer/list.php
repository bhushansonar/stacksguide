<div class="container top">
    ...
    <ul class="breadcrumb">
        ...
        <li class="active">
            Warning Disclaimer<?php //echo ucfirst($this->uri->segment(2));            ?>
        </li>
    </ul>
    ...
    if ($this->session->flashdata('flash_message')) {
        if ($this->session->flashdata('flash_message') == 'add') {
            ...
            echo '<strong>Well done!</strong> new Warning Disclaimer created with success.';
            echo '</div>';
        } else if ($this->session->flashdata('flash_message') == 'update') {
            echo '<div class="alert alert-success">';
            echo '<a class="close" data-dismiss="alert">&#215;</a>';
            echo '<strong>Well done!</strong> Warning Disclaimer updated with success.';
            echo '</div>';
        } else if ($this->session->flashdata('flash_message') == 'delete') {
            echo '<div class="alert alert-success">';
            echo '<a class="close" data-dismiss="alert">&#215;</a>';
            echo '<strong>Well done!</strong> Warning Disclaimer deleted with success.';
            echo '</div>';
        } else {
...
//save the columns names in a array that we will use as filter
                ...
                foreach ($warning_disclaimer as $array) {
                ...
                echo form_open('admin/warning_disclaimer', $attributes);
...
//echo form_dropdown('order', $options_language, $order, 'class="span2"');
                ...
                if ($search_string_selected) {
                    echo '<a style="margin-left: 12px;vertical-align: middle;" class="btn" href="' . site_url("admin/warning_disclaimer/") . '">Reset</a>';
                }
                echo form_close();
                ?>
            ...
            <table class="table table-striped table-bordered table-condensed">
                ...
                <tbody>
                    <?php
                    foreach ($warning_disclaimer as $row) {
                        echo '<tr>';
                        echo '<td>' . $row['warning_disclaimer_id'] . '</td>';
                        echo '<td>' . $row['description'] . '</td>';
                        echo '<td>' . $row['image'] . '</td>';
                        echo '<td>' . $row['status'] . '</td>';
                        //html_entity_decode($row['en'])
                        echo '<td class="crud-actions">
                  <a  href="' . site_url("admin") . '/warning_disclaimer/update/' . $row['warning_disclaimer_id'] . '" class="btn btn-info">view & edit</a>
                  <a href="' . site_url("admin") . '/warning_disclaimer/delete/' . $row['warning_disclaimer_id'] . '" class="btn btn-danger complexConfirm">delete</a>
                </td>';
                        echo '</tr>';
                    }
                    ?>
