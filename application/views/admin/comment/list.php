...
//save the columns names in a array that we will use as filter
                ...
                echo form_open('admin/comment', $attributes);
                ...
                <select name="order" class="span2">
                    ...
                    <option <?php echo ($order == 'blog.title') ? 'selected="selected"' : "" ?>  value="blog.title">Blog</option>
...
//echo form_dropdown('order', $options_language, $order, 'class="span2"');
                ...
                if ($search_string_selected) {
                    echo '<a style="margin-left: 12px;vertical-align: middle;" class="btn" href="' . site_url("admin/comment/") . '">Reset</a>';
                }
                echo form_close();
                ?>
            ...
            <table class="table table-striped table-bordered table-condensed">
                ...
                <tbody>
                    ...
                    foreach ($comment as $row) {
                        ...
                        echo '<td>' . $status . '</td>';
                        //htmltity_decode($row['en'])
                        echo '<td style="text-align:center;" class="crud-actions">
                  <a href="' . site_url("admin") . '/comment/status/' . $row['comment_id'] . '" class="btn btn-info">Click to ' . $statusbtn . '</a>
				  <a href="' . site_url("admin") . '/comment/update/' . $row['comment_id'] . '" class="btn btn-primary">Edit</a>
				   <a href="' . site_url("admin") . '/comment/delete/' . $row['comment_id'] . '" class="btn btn-danger complexConfirm">delete</a>
				  </td>';
                        echo '</tr>';
                    }
                    ?>
