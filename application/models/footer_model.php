...
class footer_model extends CI_Model {
    ...
    public function get_footer_by_block_name($block_name) {
        $this->db->select('*');
        $this->db->from('footer');
    ...
    public function get_footer_by_field($field, $value) {
        $this->db->select('*');
        $this->db->from('footer');
     ...
     * @return array
     */
    public function get_footer($search_string = null, $order = null, $order_type = 'DESC', $limit_start = null, $limit_end = null, $wherestatus = null) {
        ...
        $this->db->group_by('footer_id');

        if ($order) {
            $this->db->order_by($order, $order_type);
        } else {
            $this->db->order_by('footer_id', $order_type);
        }
     ...
     * @return int
     */
    function count_footer($search_string = null, $order = null) {
        $this->db->select('*');
        $this->db->from('footer');
     ...
     * @return boolean
     */
    function store_footer($data) {
        $insert = $this->db->insert('footer', $data);
        return $insert;
    }
    ...
    public function get_footer_front() {
        $this->db->select('*');
        $this->db->from('footer');
        $this->db->where('status', "Active");
        $query = $this->db->get();
